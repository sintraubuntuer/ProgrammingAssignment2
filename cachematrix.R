...
makeCacheMatrix <- function(x = matrix()) {
        ...
        set <- function(matY) {
               ...
               #    matY  - the matrix we want to set as the 'main' matrix of makeCacheMatrix function  
                ...
                theMat <<- matY      #  the <<- operator is used because we're assigning to a variable 
                invMat <<- NULL      #  that isn't defined inside the set function
        ...
        setinverse <- function(mInv=solve(theMat)) { 
                ...
                #  if no argument is passed , the function automatically calculates the inverse 
                #  of theMat and stores it in invMat
        ...
        # The list labels are equal to the function names , so that they are acessible to the outside
        # with the same names they have inside
        # it's an elegant way of making the inner functions accessible to the outside , making  makeCacheMatrix
        # similar to a class  with public methods : set , get , setinverse and getinverse
...
cacheSolve <- function(x, ...) {
        ...
        mat <- x$get()         # if the inverse matrix hasn't yet been calculated gets the matrix 'theMat'
                               # using the get function of the makeCacheMatrix object  
        mInv <- solve(mat, ...)   # then calculates the inverse
        x$setinverse(mInv)        # and stores it in the makeCacheMatrix object ( using the setinverse function) 
                                  # for later (more efficient) use 
